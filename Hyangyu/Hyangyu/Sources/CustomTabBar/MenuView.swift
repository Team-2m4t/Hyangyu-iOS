...
public protocol CustomMenuViewDelegate: class {
    func menuView(_ menuView: CustomMenuView, didSelectedItemAt index: Int)
}

public class CustomMenuView: UIView {
    ...
    private let screenBoundsWidth = UIScreen.main.bounds.width
    ...
    public weak var delegate: CustomMenuViewDelegate?
    ...
    public var itemSpace:CGFloat = (UIScreen.main.bounds.width - 90 ) / 3 {
        didSet {
            stackView.spacing = itemSpace
            layoutIfNeeded()
            layoutSlider()
    ...
    private var selectedTextColor = UIColor.primary
