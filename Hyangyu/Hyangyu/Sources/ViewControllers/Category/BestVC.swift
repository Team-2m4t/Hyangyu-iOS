...
class BestVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        registerXib()
        getCategory(order: "popularity", page: currentPage)
    }
...
extension BestVC : UICollectionViewDataSource
{
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if indexPath.row == allData.displays.count - 1 && !isLast {
            currentPage += 1
            self.getCategory(order: "popularity", page: currentPage)
        }
...
extension BestVC
{
    @objc func getCategory(order: String, page: Int) {
        CategoryAPI.shared.getCategory(order: "popularity", page: currentPage) { response in
            switch(response)
            {
